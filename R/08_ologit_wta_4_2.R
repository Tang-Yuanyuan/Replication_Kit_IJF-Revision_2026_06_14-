...
model_demo_car <- fit_polr_wta("wta_car", base_demos, "caruse", train_car)
model_demo_elec <- fit_polr_wta("wta_elec", base_demos, "conditioner1month", train_elec)
model_demo_green <- fit_polr_wta("wta_green", base_demos, "mainuseelec", train_green)

model_all_car <- fit_polr_wta("wta_car", base_all, "caruse", train_car)
model_all_elec <- fit_polr_wta("wta_elec", base_all, "conditioner1month", train_elec)
model_all_green <- fit_polr_wta("wta_green", base_all, "mainuseelec", train_green)
...
write.csv(wta_preds_demos, file.path(data_dir, "wta_preds_demos.csv"), row.names = FALSE)
write.csv(wta_preds_all, file.path(data_dir, "wta_preds_all.csv"), row.names = FALSE)

message("Ordered-logit WTA predictions written to: ", data_dir)
