...
sim_subdir   <- if (length(args) >= 3) args[[3]] else "empirical4.3_weighted"
...
fit_glm_logit_weighted <- function(outcome, controls, exposure, data) {
  glm(
...
model_demo_car   <- fit_glm_logit_weighted("y_car",   base_demos, "caruse",            train_car)
model_demo_elec  <- fit_glm_logit_weighted("y_elec",  base_demos, "conditioner1month", train_elec)
model_demo_green <- fit_glm_logit_weighted("y_green", base_demos, "mainuseelec",       train_green)
model_all_car    <- fit_glm_logit_weighted("y_car",   base_all,  "caruse",            train_car)
model_all_elec   <- fit_glm_logit_weighted("y_elec",  base_all,  "conditioner1month", train_elec)
model_all_green  <- fit_glm_logit_weighted("y_green", base_all,  "mainuseelec",       train_green)
...
train_wta_car$weights   <- train_wta_car$weights   / mean(train_wta_car$weights)
train_wta_elec  <- subset(train_wta, conditionernumber == 1)
train_wta_elec$weights  <- train_wta_elec$weights  / mean(train_wta_elec$weights)
train_wta_green <- subset(train_wta, energy_consume2020 > 1000)
train_wta_green$weights <- train_wta_green$weights / mean(train_wta_green$weights)

model_demo_car_wta   <- fit_polr_wta_weighted("wta_car",   base_demos, "caruse",            train_wta_car)
model_demo_elec_wta  <- fit_polr_wta_weighted("wta_elec",  base_demos, "conditioner1month", train_wta_elec)
model_demo_green_wta <- fit_polr_wta_weighted("wta_green", base_demos, "mainuseelec",       train_wta_green)
model_all_car_wta    <- fit_polr_wta_weighted("wta_car",   base_all,  "caruse",            train_wta_car)
model_all_elec_wta   <- fit_polr_wta_weighted("wta_elec",  base_all,  "conditioner1month", train_wta_elec)
model_all_green_wta  <- fit_polr_wta_weighted("wta_green", base_all,  "mainuseelec",       train_wta_green)
...
message("Weighted simulated logit and ordered-logit predictions written to: ", sim_dir)
